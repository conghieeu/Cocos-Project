(function () {
  function boot() {
    ...
    let { RESOURCES, INTERNAL, MAIN, START_SCENE } =
  ...
  if (window.document) {
    ...
    var loadImage = function (nameOrUrl, options, onComplete) {
      // Đảm bảo nameOrUrl là local resource
      if (nameOrUrl.startsWith('http')) {
        nameOrUrl = 'resources/images/' + nameOrUrl.split('/').pop();
      }
    ...
    var loadAudio = (url, options, onComplete) => {
      // Đảm bảo url là local resource
      if (url.startsWith('http')) {
        url = 'resources/audio/' + url.split('/').pop();
      }
    ...
    var splash = document.getElementById("splash");
    splash.style.display = "block";
    cc.macro.CLEANUP_IMAGE_CACHE = true; // Xóa cache hình ảnh
    cc.assetManager.downloader.maxConcurrency = 2; // Giới hạn số lượng tải đồng thời
    cc.assetManager.downloader.maxRequestsPerFrame = 2; // Giới hạn số request mỗi frame
